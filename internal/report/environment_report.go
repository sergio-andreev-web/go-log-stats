...
// EnvironmentReport aggregates events by environment.
...
func NewEnvironmentReport() *EnvironmentReport {
	return &EnvironmentReport{buckets: make(map[string]*Bucket)}
}

func (report *EnvironmentReport) Add(event Event) {
	key := strings.TrimSpace(event.Environment)
	if key == "" {
		return
	}
...
func (report *EnvironmentReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *EnvironmentReport) Total() int {
	return report.total
}

func (report *EnvironmentReport) Unique() int {
	return len(report.buckets)
}

func (report *EnvironmentReport) Find(key string) (Bucket, bool) {
...
func (report *EnvironmentReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *EnvironmentReport) Merge(other *EnvironmentReport) {
...
func (report *EnvironmentReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *EnvironmentReport) JSON(limit int) ([]byte, error) {
