...
// StatusReport aggregates events by status.
...
func NewStatusReport() *StatusReport {
	return &StatusReport{buckets: make(map[string]*Bucket)}
}

func (report *StatusReport) Add(event Event) {
	key := strings.TrimSpace(event.Status)
	if key == "" {
		return
	}
...
func (report *StatusReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *StatusReport) Total() int {
	return report.total
}

func (report *StatusReport) Unique() int {
	return len(report.buckets)
}

func (report *StatusReport) Find(key string) (Bucket, bool) {
...
func (report *StatusReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *StatusReport) Merge(other *StatusReport) {
...
func (report *StatusReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *StatusReport) JSON(limit int) ([]byte, error) {
