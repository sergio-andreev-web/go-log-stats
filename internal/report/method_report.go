...
// MethodReport aggregates events by method.
type MethodReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewMethodReport() *MethodReport {
	return &MethodReport{buckets: make(map[string]*Bucket)}
}

func (report *MethodReport) Add(event Event) {
	key := strings.TrimSpace(event.Method)
	if key == "" {
		return
	}
...
func (report *MethodReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *MethodReport) Total() int {
	return report.total
}

func (report *MethodReport) Unique() int {
	return len(report.buckets)
}

func (report *MethodReport) Find(key string) (Bucket, bool) {
...
func (report *MethodReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *MethodReport) Merge(other *MethodReport) {
...
func (report *MethodReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *MethodReport) JSON(limit int) ([]byte, error) {
