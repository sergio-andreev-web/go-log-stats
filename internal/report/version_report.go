...
// VersionReport aggregates events by version.
...
func NewVersionReport() *VersionReport {
	return &VersionReport{buckets: make(map[string]*Bucket)}
}

func (report *VersionReport) Add(event Event) {
	key := strings.TrimSpace(event.Version)
	if key == "" {
		return
	}
...
func (report *VersionReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *VersionReport) Total() int {
	return report.total
}

func (report *VersionReport) Unique() int {
	return len(report.buckets)
}

func (report *VersionReport) Find(key string) (Bucket, bool) {
...
func (report *VersionReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *VersionReport) Merge(other *VersionReport) {
...
func (report *VersionReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *VersionReport) JSON(limit int) ([]byte, error) {
