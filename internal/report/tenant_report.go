...
// TenantReport aggregates events by tenant.
type TenantReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewTenantReport() *TenantReport {
	return &TenantReport{buckets: make(map[string]*Bucket)}
}

func (report *TenantReport) Add(event Event) {
	key := strings.TrimSpace(event.Tenant)
	if key == "" {
		return
	}
...
func (report *TenantReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *TenantReport) Total() int {
	return report.total
}

func (report *TenantReport) Unique() int {
	return len(report.buckets)
}

func (report *TenantReport) Find(key string) (Bucket, bool) {
...
func (report *TenantReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *TenantReport) Merge(other *TenantReport) {
...
func (report *TenantReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *TenantReport) JSON(limit int) ([]byte, error) {
