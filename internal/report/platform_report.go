...
// PlatformReport aggregates events by platform.
...
func NewPlatformReport() *PlatformReport {
	return &PlatformReport{buckets: make(map[string]*Bucket)}
}

func (report *PlatformReport) Add(event Event) {
	key := strings.TrimSpace(event.Platform)
	if key == "" {
		return
	}
...
func (report *PlatformReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *PlatformReport) Total() int {
	return report.total
}

func (report *PlatformReport) Unique() int {
	return len(report.buckets)
}

func (report *PlatformReport) Find(key string) (Bucket, bool) {
...
func (report *PlatformReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *PlatformReport) Merge(other *PlatformReport) {
...
func (report *PlatformReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *PlatformReport) JSON(limit int) ([]byte, error) {
