...
// RegionReport aggregates events by region.
type RegionReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewRegionReport() *RegionReport {
	return &RegionReport{buckets: make(map[string]*Bucket)}
}

func (report *RegionReport) Add(event Event) {
	key := strings.TrimSpace(event.Region)
	if key == "" {
		return
	}
...
func (report *RegionReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *RegionReport) Total() int {
	return report.total
}

func (report *RegionReport) Unique() int {
	return len(report.buckets)
}

func (report *RegionReport) Find(key string) (Bucket, bool) {
...
func (report *RegionReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *RegionReport) Merge(other *RegionReport) {
...
func (report *RegionReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *RegionReport) JSON(limit int) ([]byte, error) {
