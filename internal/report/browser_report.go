...
// BrowserReport aggregates events by browser.
type BrowserReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewBrowserReport() *BrowserReport {
	return &BrowserReport{buckets: make(map[string]*Bucket)}
}

func (report *BrowserReport) Add(event Event) {
	key := strings.TrimSpace(event.Browser)
	if key == "" {
		return
	}
...
func (report *BrowserReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *BrowserReport) Total() int {
	return report.total
}

func (report *BrowserReport) Unique() int {
	return len(report.buckets)
}

func (report *BrowserReport) Find(key string) (Bucket, bool) {
...
func (report *BrowserReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *BrowserReport) Merge(other *BrowserReport) {
...
func (report *BrowserReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *BrowserReport) JSON(limit int) ([]byte, error) {
