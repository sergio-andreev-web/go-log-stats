...
// WorkerReport aggregates events by worker.
type WorkerReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewWorkerReport() *WorkerReport {
	return &WorkerReport{buckets: make(map[string]*Bucket)}
}

func (report *WorkerReport) Add(event Event) {
	key := strings.TrimSpace(event.Worker)
	if key == "" {
		return
	}
...
func (report *WorkerReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *WorkerReport) Total() int {
	return report.total
}

func (report *WorkerReport) Unique() int {
	return len(report.buckets)
}

func (report *WorkerReport) Find(key string) (Bucket, bool) {
...
func (report *WorkerReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *WorkerReport) Merge(other *WorkerReport) {
...
func (report *WorkerReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *WorkerReport) JSON(limit int) ([]byte, error) {
