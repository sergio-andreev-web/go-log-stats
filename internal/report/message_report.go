...
// MessageReport aggregates events by message.
type MessageReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewMessageReport() *MessageReport {
	return &MessageReport{buckets: make(map[string]*Bucket)}
}

func (report *MessageReport) Add(event Event) {
...
func (report *MessageReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *MessageReport) Total() int {
	return report.total
}

func (report *MessageReport) Unique() int {
	return len(report.buckets)
}

func (report *MessageReport) Find(key string) (Bucket, bool) {
...
func (report *MessageReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *MessageReport) Merge(other *MessageReport) {
...
func (report *MessageReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *MessageReport) JSON(limit int) ([]byte, error) {
