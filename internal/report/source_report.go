...
// SourceReport aggregates events by source.
type SourceReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewSourceReport() *SourceReport {
	return &SourceReport{buckets: make(map[string]*Bucket)}
}

func (report *SourceReport) Add(event Event) {
	key := strings.TrimSpace(event.Source)
	if key == "" {
		return
	}
...
func (report *SourceReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *SourceReport) Total() int {
	return report.total
}

func (report *SourceReport) Unique() int {
	return len(report.buckets)
}

func (report *SourceReport) Find(key string) (Bucket, bool) {
...
func (report *SourceReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *SourceReport) Merge(other *SourceReport) {
...
func (report *SourceReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *SourceReport) JSON(limit int) ([]byte, error) {
