...
// LevelReport aggregates events by level.
type LevelReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewLevelReport() *LevelReport {
	return &LevelReport{buckets: make(map[string]*Bucket)}
}

func (report *LevelReport) Add(event Event) {
	key := strings.TrimSpace(event.Level)
	if key == "" {
		return
	}
...
func (report *LevelReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *LevelReport) Total() int {
	return report.total
}

func (report *LevelReport) Unique() int {
	return len(report.buckets)
}

func (report *LevelReport) Find(key string) (Bucket, bool) {
...
func (report *LevelReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *LevelReport) Merge(other *LevelReport) {
...
func (report *LevelReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *LevelReport) JSON(limit int) ([]byte, error) {
