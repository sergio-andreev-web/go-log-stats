...
// CategoryReport aggregates events by category.
...
func NewCategoryReport() *CategoryReport {
	return &CategoryReport{buckets: make(map[string]*Bucket)}
}

func (report *CategoryReport) Add(event Event) {
	key := strings.TrimSpace(event.Category)
	if key == "" {
		return
	}
...
func (report *CategoryReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *CategoryReport) Total() int {
	return report.total
}

func (report *CategoryReport) Unique() int {
	return len(report.buckets)
}

func (report *CategoryReport) Find(key string) (Bucket, bool) {
...
func (report *CategoryReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *CategoryReport) Merge(other *CategoryReport) {
...
func (report *CategoryReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *CategoryReport) JSON(limit int) ([]byte, error) {
