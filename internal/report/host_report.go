...
// HostReport aggregates events by host.
type HostReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewHostReport() *HostReport {
	return &HostReport{buckets: make(map[string]*Bucket)}
}

func (report *HostReport) Add(event Event) {
	key := strings.TrimSpace(event.Host)
	if key == "" {
		return
	}
...
func (report *HostReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *HostReport) Total() int {
	return report.total
}

func (report *HostReport) Unique() int {
	return len(report.buckets)
}

func (report *HostReport) Find(key string) (Bucket, bool) {
...
func (report *HostReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *HostReport) Merge(other *HostReport) {
...
func (report *HostReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *HostReport) JSON(limit int) ([]byte, error) {
