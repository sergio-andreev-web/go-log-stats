...
// PathReport aggregates events by path.
type PathReport struct {
	buckets map[string]*Bucket
	total   int
}

func NewPathReport() *PathReport {
	return &PathReport{buckets: make(map[string]*Bucket)}
}

func (report *PathReport) Add(event Event) {
	key := strings.TrimSpace(event.Path)
	if key == "" {
		return
	}
...
func (report *PathReport) Rows(limit int) []Bucket {
	return Rank(report.buckets, limit)
}

func (report *PathReport) Total() int {
	return report.total
}

func (report *PathReport) Unique() int {
	return len(report.buckets)
}

func (report *PathReport) Find(key string) (Bucket, bool) {
...
func (report *PathReport) Top() (Bucket, bool) {
	rows := report.Rows(1)
	if len(rows) == 0 {
		return Bucket{}, false
	}
...
func (report *PathReport) Merge(other *PathReport) {
...
func (report *PathReport) Reset() {
	report.buckets = make(map[string]*Bucket)
	report.total = 0
}

func (report *PathReport) JSON(limit int) ([]byte, error) {
